...
program
  ...
  .option('--api-prefix <prefix>', 'Additional path prefix for API routes (e.g., "rest" for /api/rest)')
  .option('-p, --prisma-import <path>', 'Import path for Prisma client', '@/lib/prisma')
  ...
  .action(async (options) => {
    try {
      ...
      const defaultOutputPath = usesSrcDir ? `./src/app/api${apiPrefix}` : `./app/api${apiPrefix}`
      ...
      if (!existsSync(config.schemaPath)) {
        console.error(chalk.red(`L Prisma schema not found at: ${config.schemaPath}`))
        process.exit(1)
      }
      
      // Parse schema
      const schema = await parsePrismaSchema(config.schemaPath)
      console.log(chalk.green(` Parsed ${schema.models.length} models from schema`))
      ...
      for (const model of models) {
        ...
        const modelDir = join(config.outputPath, model.name.toLowerCase() + 's')
        ...
        if (routesExist) {
          if (options.skipExisting) {
            console.log(chalk.yellow(`  ⏭️  Skipping existing routes for ${model.name}`))
            skippedCount++
            continue
          } else if (!options.force) {
            console.log(chalk.red(`  ❌ Routes already exist for ${model.name}`))
            existingModels.push(model.name)
            continue
          }
        }
        
        console.log(chalk.cyan(`  ${options.dryRun ? '[DRY RUN]' : '✓'} Generating routes for ${model.name}...`))
        ...
        if (!options.dryRun) {
          ...
          const listHandler = generateRouteHandler(model, 'list', config.prismaImportPath)
          const createHandler = generateRouteHandler(model, 'create', config.prismaImportPath)
          
          const combinedListCreate = addGeneratorHeader(combineHandlers(listHandler, createHandler))
          ...
          const getHandler = generateRouteHandler(model, 'get', config.prismaImportPath)
          const updateHandler = generateRouteHandler(model, 'update', config.prismaImportPath)
          const deleteHandler = generateRouteHandler(model, 'delete', config.prismaImportPath)
          
          const combinedItem = addGeneratorHeader(combineHandlers(getHandler, updateHandler, deleteHandler))
      ...
      // Summary messages
      if (options.dryRun) {
        console.log(chalk.blue(`\n🔍 DRY RUN: Would generate ${generatedCount} route${generatedCount !== 1 ? 's' : ''}`))
      } else {
        console.log(chalk.green(`\n✅ Successfully generated ${generatedCount} route${generatedCount !== 1 ? 's' : ''}!`))
      }
      
      if (skippedCount > 0) {
        console.log(chalk.yellow(`⏭️  Skipped ${skippedCount} existing route${skippedCount !== 1 ? 's' : ''}`))
      }
      
      if (existingModels.length > 0 && !options.force && !options.skipExisting) {
        console.log(chalk.red(`\n⚠️  Routes already exist for: ${existingModels.join(', ')}`))
        console.log(chalk.yellow(`   Use --force to overwrite or --skip-existing to skip them`))
      }
...
function addGeneratorHeader(content: string): string {
  ...
  const header = `// Generated by prisma-rest-generator v${version}
